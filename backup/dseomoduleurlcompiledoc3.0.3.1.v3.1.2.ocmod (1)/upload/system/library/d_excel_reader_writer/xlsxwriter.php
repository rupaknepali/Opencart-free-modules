...
namespace d_excel_reader_writer;
...
class XLSXWriter {
	...
	protected function initializeSheet($sheet_name) {
        ...
        if (!empty($this->columnWidths)) {
            foreach($this->columnWidths as $k => $v) {
                $idx = $k+1;
                $sheet->file_writer->write('<col collapsed="false" hidden="false" max="'.$idx.'" min="'.$idx.'" style="0" width="'.$v.'" customWidth="1"/>');      
            }
			
            $sheet->file_writer->write('<col collapsed="false" hidden="false" max="1025" min="'.(count($this->columnWidths)+1).'" style="0" width="'.$this->defaultColumnWidth.'" customWidth="1"/>');
        } else {
            $sheet->file_writer->write('<col collapsed="false" hidden="false" max="1025" min="1" style="0" width="'.$this->defaultColumnWidth.'" customWidth="1"/>');
        }
	...
	public function writeSheetHeader($sheet_name, array $header_types, $suppress_row = false) {
		...
		if (!$suppress_row) {
			...
			$sheet->file_writer->write('<row collapsed="false" customFormat="false" customHeight="1" hidden="false" ht="30" outlineLevel="0" r="' . (1) . '">');
	...
	public function writeSheetRow($sheet_name, array $row, $style = null) {
		...
		$sheet->file_writer->write('<row collapsed="false" customFormat="false" customHeight="1" hidden="false" ht="30" outlineLevel="0" r="' . ($sheet->row_count + 1) . '">');
